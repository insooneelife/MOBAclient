...
void GoalTraverseEdge::activate()
{
	...
	{
		_owner->setMaxSpeed(Prm.max_speed * 0.2);
	}
	...
	{
		_owner->setMaxSpeed(Prm.max_speed * 0.6);
	}
...
int GoalTraverseEdge::process()
{
	...
	{
		if (_owner->isAtPosition(_edge.getDestination()))
		{
			_status = COMPLETED;
		}
	}
...
void GoalTraverseEdge::terminate()
{
	...
	_owner->setMaxSpeed(Prm.max_speed);
}
...
void GoalTraverseEdge::render()
{
	if (Prm.RenderPathLine) 
	{
		Debug.getPathLinePanel()->
			drawLine(_owner->getPos(), _edge.getDestination(), cocos2d::Color4F::BLUE);

		Debug.getPathLinePanel()->drawCircle(
			_edge.getDestination(),
			10,
			(float)(2 * M_PI),
