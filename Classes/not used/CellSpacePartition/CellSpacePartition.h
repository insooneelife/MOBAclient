...
struct Cell
{
	...
	cocos2d::Label*		cell_label;

	Cell(
		cocos2d::Layer* const	view_layer,
		cocos2d::Vec2			botleft,
		cocos2d::Vec2			topright)
		:
		bounding_box(AABBox2D(botleft, topright))
	{
		cell_label = Label::createWithTTF("", Prm.FontFolderName + "/" + "arial" + ".ttf", 12);
		cell_label->setGlobalZOrder(LABEL_ZORDER);
		cell_label->setTag(RenderUtil::CELL_LABEL);
		cell_label->setPosition(cocos2d::Vec2(botleft.x + 10, botleft.y + 10));
		cell_label->setColor(Color3B::WHITE);

		view_layer->addChild(cell_label);
	}

	~Cell()
	{
		cell_label->removeFromParent();
	}

	void renderEntityOn() 
	{
		Debug.getCellBoxPanel()->drawRect(
			cocos2d::Vec2(bounding_box.getLeft() + 5, bounding_box.getBottom() + 5),
			cocos2d::Vec2(bounding_box.getRight() - 5, bounding_box.getTop() - 5),
			cocos2d::Color4F::YELLOW);

		std::stringstream ss;
		ss << members.size();
		cell_label->setString(ss.str());
	}
	void renderEntityOff() 
	{
		Debug.getCellBoxPanel()->drawRect(
			cocos2d::Vec2(bounding_box.getLeft(), bounding_box.getBottom()),
			cocos2d::Vec2(bounding_box.getRight(), bounding_box.getTop()),
